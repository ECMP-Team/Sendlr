...
/**
 * Generates email content for multiple clients using a custom prompt
 * @param {string} customPrompt - Custom prompt template for email generation
 * @param {Array} clientDataArray - Array of client data objects
 * @returns {Promise<Array>} Array of generated email content objects
 */
async function writeMail(customPrompt = "", clientDataArray = []) {
  if (!Array.isArray(clientDataArray)) {
    clientDataArray = [clientDataArray]; // Convert single object to array for consistency
  }
  ...
  for (const clientData of clientDataArray) {
    try {
      ...
      const defaultPrompt = `You are an expert AI email marketing copywriter specializing in high-conversion email campaigns. Your task is to craft a persuasive marketing email for ECMP, an AI-powered email campaign management platform that automates email writing and bulk sending.
...
Compliance: Ensure a professional tone that follows email marketing regulations (no spammy language).`;
      ...
      const message = `${customPrompt || defaultPrompt}
